...
module.exports = class extends Base {
  ...
  async categoryAction() {
    ...
    const brotherCategory = await this.model('menu_category').where({store_id: this.get('id')}).select();
  ...
  async categorystoreAction() {
    ...
    const currentCategory = await model.where({id: this.get('id')}).find();
    // const parentCategory = await model.where({id: currentCategory.parent_id}).find();
    const brotherCategory = await this.model('category').select();
  ...
  async listgoodAction() {
    const categoryId = this.get('categoryId');
    const store_id = this.get('store_id');
    const page = this.get('page');
    const size = this.get('size');
    const sort = this.get('sort');
    const order = this.get('order');
    ...
    if (!think.isEmpty(categoryId)) {
      whereMap.menu_category_id	 = categoryId;
    }

    if (!think.isEmpty(store_id)) {
      whereMap.store_id = store_id;
    }
