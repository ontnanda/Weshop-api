...
module.exports = class extends Base {
  ...
  listAction() {
    ...
    return _asyncToGenerator(function* () {
      const orderList = yield _this.model('order').where({ user_id: think.userId }).page(1, 10).countSelect();
      const newOrderList = [];
  ...
  detailAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      const orderInfo = yield _this2.model('order').where({ user_id: 1, id: orderId }).find();
  ...
  submitAction() {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this3.model('cart').clearBuyGoods();
